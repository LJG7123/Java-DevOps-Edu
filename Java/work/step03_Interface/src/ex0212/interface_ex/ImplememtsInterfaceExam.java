...
interface InterfaceExam01 extends InterfaceExam00{
	final int i = 767;
	int k = 999; // public, static, final 접근 제한자만 사용 가능
...
interface InterfaceExam02{
	int j =747;
	int privateInt = 8; // private 사용 불가
	void interfaceExam02(int j); // private 접근 제한자를 사용하려면 구현부가 필요하고, 아니라면 private 제한자는 사용 불가
...
// implements 한 인터페이스의 메소드는 반드시 구현해야함
...
// SuperClassExam 클래스는 인터페이스가 아니기 때문에 extends 를 사용해야 상속 가능
