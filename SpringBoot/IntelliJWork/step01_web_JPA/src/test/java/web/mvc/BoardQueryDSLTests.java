...
public class BoardQueryDSLTests {
	...
	public void test3() {
		...
		List<Board> list =
				jpaQueryFactory
						.selectFrom(board)
						.where(board.bno.loe(30L).or(board.title.eq("제목80")))
						.fetch();
		System.out.println("---------------");
		list.forEach(System.out::println);
	}
	...
	public void test5() {
		...
		long re = jpaQueryFactory
				.update(board)
				.set(board.writer, "이쁜이2")
				.set(board.title, "수정되나?2")
